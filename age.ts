...
const calculateAge = (day: number, month: number, year: number): { years: number, months: number, days: number } => {
...
const displayTable = (day: number, month: number, year: number, age: { years: number, months: number, days: number }) => {
    ...
    table.push(
        [chalk.green('Day of Birth'), chalk.yellow(day.toString())],
        [chalk.green('Month of Birth'), chalk.yellow(months[month - 1])],
        [chalk.green('Year of Birth'), chalk.yellow(year.toString())],
        [chalk.green('Age in Years'), chalk.yellow(age.years.toString())],
        [chalk.green('Age in Months'), chalk.yellow(age.months.toString())],
        [chalk.green('Age in Days'), chalk.yellow(age.days.toString())]
    );
...
const main = async () => {
    ...
    while (continueFlag) {
        ...
        if (choice === 'Exit') {
            ...
            exitTable.push(
                [chalk.green(`Your Date of Birth: ${day} ${months[month - 1]} ${year}`)],
                [chalk.green(`Your Age: ${age.years} Years, ${age.months} Months, ${age.days} Days`)]
            );
